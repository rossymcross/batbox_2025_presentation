...
export const DaikinExperienceSlide: React.FC<DaikinExperienceSlideProps> = ({ onNext, onPrev }) => {
  ...
  const smallStars = useMemo(() => 
    Array.from({ length: 150 }, () => ({
      ...
      delay: Math.random() * 2,
      opacityRange: [Math.random() * 0.3 + 0.3, Math.random() * 0.8 + 0.2, Math.random() * 0.3 + 0.3] as [number, number, number]
    })), 
  []);
  ...
  const mediumStars = useMemo(() => 
    Array.from({ length: 80 }, () => ({
      ...
      delay: Math.random() * 2,
      opacityRange: [Math.random() * 0.4 + 0.3, Math.random() * 0.9 + 0.1, Math.random() * 0.4 + 0.3] as [number, number, number]
    })), 
  []);
  ...
  return (
    <div className="w-screen h-screen bg-[#050505] text-white p-10 flex flex-col overflow-hidden relative selection:bg-cyan-500/5">
      ...
      {/* Distant Stars Layer 1 - Small stars */}
      ...
      {shootingStars.map((star, i) => (
        ...
        >
          {/* Shooting star tail */}
          <div 
            className="absolute top-0 left-0 w-[80px] h-[1px] origin-left -rotate-[30deg]"
            style={{
      ...
      {/* Nebula/Galaxy Effect - Bottom Right */}
      <motion.div
        className="absolute pointer-events-none"
        style={{
          bottom: '10%',
        ...
        >
          <div className="flex items-center gap-5 mb-2">
            ...
            <div>
              ...
              <p className="text-xl text-gray-300 font-semibold tracking-wide mt-1">First Custom Game • 2026 Q1 Sizzle Reels</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          >
            ...
            <motion.button
              ...
              className="w-12 h-12 bg-black/60 border border-white/20 rounded-full backdrop-blur-md flex items-center justify-center text-white hover:bg-black/80 hover:border-cyan-400/60 transition-all duration-300"
          ...
          <div className="relative w-full h-full flex items-center justify-center">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                >
                  <motion.div
                    className={`w-24 h-24 rounded-full bg-white/10 backdrop-blur-md border-2 border-white/40 flex items-center justify-center relative overflow-hidden ${currentVideo.videoSrc ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'}`}
                    style={{
                      boxShadow: '0 0 40px rgba(6, 182, 212, 0.4), 0 0 80px rgba(6, 182, 212, 0.2)'
                    }}
                    whileHover={currentVideo.videoSrc ? { scale: 1.15 } : {}}
                    whileTap={currentVideo.videoSrc ? { scale: 0.95 } : {}}
                  >
                    {/* Pulsing ring effect */}
